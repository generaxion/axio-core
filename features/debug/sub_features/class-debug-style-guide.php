 ...
 */
class Axio_Core_Debug_Style_Guide extends Axio_Core_Sub_Feature {

  public function setup() {

    // var: key
    $this->set('key', 'axio_core_debug_style_guide');
   ...
   */
  public function run() {
    add_filter('the_content', array($this, 'axio_core_style_guide_markup'));
  }

  /**
   * Add style guide test markup if the GET parameter "?ac-debug=styleguide" is present in the url.
   * The axio_core_custom_markup filter makes it possible to replace the default with custom markup
   *
  ...
  public static function axio_core_style_guide_markup($content) {
    if (isset($_GET['ac-debug']) && $_GET['ac-debug'] == 'styleguide') {
      ...
      $content = apply_filters('axio_core_custom_markup', $content);
